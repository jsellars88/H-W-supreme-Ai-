...
def run_all():
    with client() as c:
        ...
        ok("11 authorize_t1", r.json().get("outcome") == "ALLOW", f"body={r.text[:200]}")

        # 10. Decision replay
        r = c.get(f"/v1/decisions/sensing/{n1}", headers=H)
        ok("12 decision_replay", r.json().get("outcome") == "ALLOW", f"body={r.text[:200]}")
        ...
        ok("13 replay_protection", r.json().get("outcome") == "DENY", f"body={r.text[:200]}")
        ...
        ok("16 telemetry_prometheus", "counter" in r.text or "gauge" in r.text or r.text == "")
        ...
        ops = r.json()
        ok("22 list_operators", len(ops) >= 2, f"found {len(ops)} operators")
        ...
        ok("25 hsm_manifest", len(r.json().get("slots", {})) == 4, f"body={r.text[:200]}")

        # 24. Key rotation
        r = c.post(
            "/v1/hsm/rotate",
            headers=H,
            json={"slot": "audit_sealing", "witnesses": ["trustee_a", "trustee_b"]},
        )
        ok("26 hsm_rotate", r.json().get("epoch") == 2, f"body={r.text[:200]}")
        ...
        ok("28 boot_attestation", r.json().get("baseline") is not None, f"body={r.text[:200]}")
        ...
        r = c.post("/v1/sessions", headers=H, json={"operator_pubkey": bravo_pub})
        ok("31 revoked_session_denied", r.status_code in (400, 401), f"status={r.status_code}")

        # 30. Invariants still hold after all operations
        r = c.get("/v1/invariants", headers=H)
        ok(
            "32 final_invariants_hold",
            r.json().get("all_invariants_hold") is True,
            f"body={r.text[:200]}",
        )
