...
KERNEL: Any | None = None  # expected: k35.WhiteSwanKernel35
...
async def auth_handler(req: Request, exc: Exception):
    return JSONResponse(
        status_code=403, content={"error": "insufficient_authority", "detail": str(exc)}
    )
...
async def op_handler(req: Request, exc: Exception):
    return JSONResponse(
        status_code=401, content={"error": "operator_not_authorized", "detail": str(exc)}
    )
...
async def gov_handler(req: Request, exc: Exception):
    return JSONResponse(
        status_code=400, content={"error": "governance_violation", "detail": str(exc)}
    )
...
class RegisterOperatorReq(BaseModel):
    ...
    geo_allowed_regions: list[str] | None = None
    geo_denied_regions: list[str] | None = None
...
def register_operator(body: RegisterOperatorReq, request: Request):
    ...
    if body.geofence or body.geo_allowed_regions or body.geo_denied_regions:
        geo = k34.GeoConstraint(
            allowed_regions=set(
                body.geo_allowed_regions or ([body.geofence] if body.geofence else [])
            ),
...
class IssueHandshakeReq(BaseModel):
    ...
    nonce: str
    operator_pubkey: str
    model_ctx: dict[str, Any] | None = None
    nonce_context: dict[str, Any] | None = None
...
def authorize(body: AuthorizeReq, request: Request):
    ...
    scope = k34.ActionScope(body.scope)
    mc = k34.ModelContext(**body.model_ctx) if body.model_ctx else None
    envelope = ker.mgi.authorize(
        scope=scope, nonce=body.nonce, expected_context=body.expected_context, model_ctx=mc
    )
    return envelope
...
def get_decision(scope: str, nonce: str, request: Request):
    ...
    row = ker.gov.db.conn.execute(
        (
            "SELECT envelope_json FROM decisions "
            "WHERE scope=? AND nonce=? ORDER BY decided_at DESC LIMIT 1"
        ),
        (scope, nonce),
    ).fetchone()
    if not row:
