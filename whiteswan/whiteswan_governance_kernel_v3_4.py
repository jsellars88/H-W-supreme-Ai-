...
try:
    ...
    class VerifyKey:
        ...
        def encode(self, encoder: Any = None) -> bytes:
            return encoder.encode(self._key) if encoder else self._key
    ...
    class SigningKey:
        ...
        def sign(self, msg: bytes) -> _SignedMessage:
            sig = hmac.new(self._key, msg, hashlib.sha256).digest()
            return _SignedMessage(sig)

        def encode(self, encoder: Any = None) -> bytes:
            return encoder.encode(self._key) if encoder else self._key

    CRYPTO_BACKEND = "hmac-sha256-sim"
...
DEFAULT_ALGORITHM = "ed25519" if CRYPTO_BACKEND == "pynacl" else "hmac-sha256-sim"
