...
Holmes & Watson Supreme AI(tm) - Curated Public Research Harvester
...
SUBTOPICS: Dict[str, List[str]] = {
    ...
    "red_teaming": [
        "red teaming", "adversarial testing", "safety eval", "eval agent",
    ],
    "robot_safety_case": [
        "ul 4600", "safety case", "functional safety", "hazard analysis",
    ],
    "sim_to_real": [
        "sim-to-real", "domain randomization", "synthetic training",
        "digital twin",
    ],
    "bvlos_rulemaking": [
        "faa bvlos", "part 108", "remote id", "uas regulation",
    ],
    "medical_imaging": [
        "beamforming", "ultrasound", "radiology", "medical imaging",
    ],
    "neurotech": [
        "brain computer interface", "neuromodulation",
        "neural prosthetics", "bci",
    ],
    "compliance": [
...
def summarize_text(text: str, max_sentences: int = 5) -> str:
    ...
    Deterministic. Not embedding-based.
    """
    ...
    best = [
        s for _, s in sorted(scored, key=lambda x: x[0], reverse=True)[:max_sentences]
    ]
...
def generate_subtopics(text: str) -> List[str]:
    lower = text.lower()
    return [
        name for name, kws in SUBTOPICS.items()
...
def extract_arxiv_id(url: str) -> str:
    m = re.search(
        r"arxiv.org/(?:abs|pdf)/([0-9]{4}\.[0-9]{4,5})(?:\.pdf)?", url
    )
...
class RobotsCache:
    ...
    def allowed(self, url: str, user_agent: str = USER_AGENT) -> bool:
        ...
        if rp is None:
            return False  # conservative: deny if robots.txt unavailable
...
class VaultIndex:
    ...
    def seen_hash(self, sha256: str) -> Optional[str]:
        row = self.db.execute(
            "SELECT capsule_id FROM capsules WHERE sha256 = ?", (sha256,)
        ).fetchone()
        return row[0] if row else None
    ...
    def add_capsule(self, capsule: EvidenceCapsule, path: str) -> None:
        self.db.execute(
            """INSERT INTO capsules
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?)""",
            (
                capsule.capsule_id, capsule.canonical_url, capsule.sha256,
                capsule.topic, capsule.title, path, capsule.fetched_at,
            ),
        )
        self.db.commit()
    ...
    def log(
        ...
        self.db.execute(
            """INSERT INTO harvest_log
               (original_url, canonical_url, status, note, harvested_at)
               VALUES (?, ?, ?, ?, ?)""",
...
class Collector:
    ...
    def __init__(
        self, timeout: int = 20, enforce_robots: bool = True
    ) -> None:
...
def search_arxiv(query: str, n: int = 5) -> List[str]:
    try:
        ...
        time.sleep(3)
        resp = requests.get(
            url, timeout=20, headers={"User-Agent": USER_AGENT}
        )
        if resp.status_code != 200:
            return []
...
class EvidenceVault:
            ...
            try:
                ...
                if q in haystack:
                    results.append({
                        ...
                        "url": (
                            data.get("canonical_url") or data.get("url")
                        ),
    ...
    def reindex(self) -> Dict[str, int]:
        """
        Rebuild the SQLite index by scanning all JSON capsules on disk.
        Use after manual file moves or index corruption.
        Returns counts: {scanned, added, skipped, errors}.
        """
        counts = {"scanned": 0, "added": 0, "skipped": 0, "errors": 0}
        for file in self._dir.rglob("*.json"):
            counts["scanned"] += 1
            try:
                data = json.loads(file.read_text(encoding="utf-8"))
                if "capsule_id" not in data or "canonical_url" not in data:
                    counts["skipped"] += 1
                    continue
                if self._index.seen_url(data["canonical_url"]):
                    counts["skipped"] += 1
                    continue
                self._index.db.execute(
                    """INSERT OR IGNORE INTO capsules
                       (capsule_id, canonical_url, sha256, topic, title,
                        path, fetched_at)
                       VALUES (?, ?, ?, ?, ?, ?, ?)""",
                    (
                        data.get("capsule_id", ""),
...
# =============================================================================
# VAULT INDEXER  (generates Markdown table of contents)
# =============================================================================

class VaultIndexer:
    """
    Walks the vault and generates a Markdown table of contents.
    One section per topic. One row per capsule.
    Includes title, URL, subtopics, date, arXiv ID if present.
    """
...
class Harvester:
    ...
    def harvest_url(
        ...
        # arXiv: escalate abs to PDF for denser text extraction
        ...
        if force_topic and topic:
            ...
            keywords = [
                kw for kw in TOPICS[topic] if kw.lower() in text.lower()
            ]
        else:
            ...
            if topic and not best_topic and scores.get(topic, 0) >= 1:
                best_topic = topic
                keywords = [
                    kw for kw in TOPICS[topic]
                    if kw.lower() in text.lower()
                ]

        if not best_topic:
            self._log("    x no topic match above threshold")
            self._vault.log(
                original_url, canon, "skipped", "no topic match"
            )
            self._stats["skipped"] += 1
            return None
        ...
        if stored:
            ...
            self._vault.log(
                original_url, capsule.canonical_url, "stored", path
            )
            self._log(f"    + [{best_topic}] {title[:60]} -> {path}")
        else:
            self._stats["skipped"] += 1
            self._vault.log(
                original_url, capsule.canonical_url, "duplicate", path
            )
    ...
    def harvest_topic(
        ...
        self._log(f"\n  [Seeds: {len(seeds)}]")
        for url in seeds:
            cap = self.harvest_url(
                url, topic=topic, force_topic=True, source_query="seed"
            )
            if cap:
                results.append(cap)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "HWS Research Harvester Pro v3.1 "
            "- Guardian Vault X ingestor"
        )
    )
    parser.add_argument(
        "--topic", choices=list(TOPICS.keys()),
        help="Harvest a specific topic",
    )
    parser.add_argument(
        "--all", action="store_true", help="Harvest all topics",
    )
    parser.add_argument(
        "--url", type=str, help="Harvest a specific URL",
    )
    parser.add_argument(
        "--search", type=str, help="Search arXiv for a query",
    )
    parser.add_argument(
        "--report", action="store_true", help="Print vault report",
    )
    parser.add_argument(
        "--find", type=str, help="Search vault for a term",
    )
    parser.add_argument(
        "--reindex", action="store_true",
        help="Rebuild SQLite index from vault JSON files",
    )
    parser.add_argument(
        "--index", action="store_true", help="Generate Markdown TOC",
    )
    parser.add_argument(
        "--limit", type=int, default=5, help="Max items per topic",
    )
    parser.add_argument(
        "--vault", type=str, default="guardian_vault_x",
        help="Vault directory",
    )
    parser.add_argument(
        "--db", type=str, default="guardian_vault_x.sqlite",
        help="SQLite index",
    )
    parser.add_argument(
        "--out", type=str, default=None, help="Output path for --index",
    )
    parser.add_argument(
    ...
    harvester = Harvester(
        vault_dir=args.vault, db_path=args.db, verbose=not args.quiet
    )
    ...
    try:
        ...
        if args.reindex:
            counts = harvester.vault.reindex()
        ...
        if args.url:
            harvester.harvest_url(
                args.url, topic=args.topic,
                force_topic=bool(args.topic),
            )
        elif args.search:
