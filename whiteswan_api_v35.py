...
def _identity_from_pubkey(pubkey_hex: str, missing_error: str = "operator_key_not_cached") -> k34.OperatorIdentity:
    sk = OP_KEYS.get(pubkey_hex)
    if not sk:
        raise HTTPException(400, missing_error)
    vk = sk.verify_key
    ...
    return k34.OperatorIdentity(kid, name, role, sk, vk)
...
def create_session(body: CreateSessionReq, request: Request):
    _check_auth(request, write=True)
    ident = _identity_from_pubkey(body.operator_pubkey, "operator_private_key_not_cached")
...
def issue_handshake(body: IssueHandshakeReq, request: Request):
    ...
    ident = _identity_from_pubkey(body.operator_pubkey)
...
# ═════════════════════════════════════════════════════════════════════
# ENDPOINT MAP (for documentation)
# ═════════════════════════════════════════════════════════════════════
# Total: 52 endpoints
#
# CORE (v3.4 carried forward):
#   GET  /v1/health                    Full system health
#   GET  /v1/invariants                8 architectural invariants
#   GET  /v1/attestation               Kernel attestation
#   GET  /v1/telemetry                 JSON telemetry
#   GET  /v1/telemetry/prometheus      Prometheus export
#   GET  /v1/vault/tail                Vault entries
#   GET  /v1/vault/chain/verify        Chain integrity
#   GET  /v1/seals                     Audit seals
#   GET  /v1/scopes                    Scope→tier map
#   GET  /v1/nonce                     Generate nonce
#   GET  /v1/policy/history            Policy versions
#   POST /v1/operators                 Register operator
#   GET  /v1/operators                 List operators
#   DEL  /v1/operators/{pubkey}        Revoke operator
#   POST /v1/sessions                  Create session
#   DEL  /v1/sessions/{sid}            Revoke session
#   POST /v1/handshakes/issue          Issue handshake
#   POST /v1/authorize                 MGI authorization
#   GET  /v1/decisions/{scope}/{nonce} Replay decision
#   POST /v1/sas/enter                 Enter SAS
#   GET  /v1/sas/status                SAS status
#   POST /v1/emergency/initiate        Emergency override
#   POST /v1/emergency/approve         Approve override
#   POST /v1/seals/create              Create seal
#   POST /v1/seals/witness             Witness seal
#   POST /v1/trustees                  Add trustee
#   GET  /v1/trustees                  List trustees
#
# NEW v3.5 SUBSYSTEMS:
#   GET  /v1/hsm/manifest              §3 HSM key manifest
#   POST /v1/hsm/rotate                §3 Key rotation
#   GET  /v1/hsm/rotations             §3 Rotation history
#   GET  /v1/boot/attestation          §4 Boot attestation
#   POST /v1/boot/reattest             §4 Runtime re-attestation
#   POST /v1/tpi/initiate              §5 TPI challenge start
#   POST /v1/tpi/complete              §5 TPI challenge complete
#   GET  /v1/tpi/{id}                  §5 TPI challenge status
#   POST /v1/federation/peers          §6 Register peer kernel
#   GET  /v1/federation/health         §6 Federation health
#   GET  /v1/federation/peers/{id}/verify §6 Verify peer
#   POST /v1/federation/quarantine     §6 Quarantine rogue peer
#   GET  /v1/federation/consensus/t4   §6 T4 consensus check
#   POST /v1/rollback/initiate         §7 CRP initiate
#   POST /v1/rollback/execute          §7 CRP execute
#   GET  /v1/rollback/history          §7 Rollback history
#   POST /v1/liveness/record           §8 Record liveness event
#   GET  /v1/liveness/check            §8 Check liveness deadlines
#   POST /v1/federation/identities     §9 Issue portable identity
#   GET  /v1/federation/identities     §9 List identities
#   GET  /v1/federation/revocations    §9 Revocation log
#   POST /v1/risk/record               §10 Record risk event
#   GET  /v1/risk/report               §10 Risk report
#   GET  /v1/risk/events               §10 Risk events
#   POST /v1/simulate/authorize        §11 Simulate authorization
#   POST /v1/simulate/sas              §11 Simulate SAS
#   POST /v1/simulate/policy-migration §11 Simulate migration
#   GET  /v1/simulate/history          §11 Simulation history
#   GET  /v1/forensics/timeline        §12 Timeline replay
#   GET  /v1/forensics/operators       §12 Operator clustering
#   GET  /v1/forensics/drift           §12 Drift analysis
#   GET  /v1/forensics/sas-causes      §12 SAS root causes
#   GET  /v1/forensics/anomalies       §12 Anomaly correlation
#   GET  /v1/forensics/report          §12 Signed report
#   GET  /v1/export/cef                §13 Full CEF export
